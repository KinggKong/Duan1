...
public class ThanhVienDao extends QLTVDao<ThanhVien, String>{
    ...
    public void insert(ThanhVien tv) {
        ...
        JDBCHelper.executeUpdate(sql, tv.getMaTV(), tv.getTenTV(), tv.getSDT(), tv.getDiaChi(),tv.getEmail(),tv.getCCCD(),tv.getNgaySinh(),tv.getNgayDK());
    }

    @Override
    public void update(ThanhVien tv) {
        String sql = "UPDATE ThanhVien SET TenTV = ?, SDT =? ,DiaChi =?, Email = ?,CCCD= ?,NgaySinh=?,NgayDK=? where MaTV = ?";
        JDBCHelper.executeUpdate(sql, tv.getTenTV(), tv.getSDT(), tv.getDiaChi(), tv.getEmail(),tv.getCCCD(),tv.getNgayDK(),tv.getNgaySinh(),tv.getMaTV());
    }
    ...
    public ThanhVien selectById(String maTV) {
        String sql = "Select MaTV,TenTV,SDT,DiaChi,Email,CCCD,NgaySinh,NgayDK from ThanhVien where MaTV = ?";
        ThanhVien tv = null;
        try {
            ResultSet rs = null;
            try {
    ...
    public List<ThanhVien> loadTrang(int indexTrang, int limit) {
        String sql = "select MaTV,TenTV,SDT,DiaChi,Email,CCCD,NgaySinh,NgayDK from ThanhVien order by MaTV offset ? rows fetch next ? rows only ";
