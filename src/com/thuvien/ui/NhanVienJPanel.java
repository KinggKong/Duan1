...
public class NhanVienJPanel extends JPanel {
	...
	public NhanVienJPanel() {
		...
		String[] columns = { "Mã NV", "Họ Tên", "SDT", "Email", "Password", "Vai Trò", "Trạng Thái" };
		Object[][] rows = {};
			...
			protected void done() {
				try {
					...
					for (NhanVien nv : list) {
						Object[] row = { nv.getMaNV(), nv.getTenNV(), nv.getSdt(), nv.getUserName(), nv.getPassWord(),
								nv.isVaiTro() ? "admin" : "nhanvien",
								nv.isTrangThai() ? "Còn Hoạt Động" : "Nghỉ hoạt động" };
						model.addRow(row);
					}
				} catch (Exception e) {
				...
				try {
					...
					if (list.size() == 0) {
						...
						for (NhanVien tg : list) {
							Object[] row = { tg.getMaNV(), tg.getTenNV(), tg.getSdt(), tg.getUserName(),
									tg.getPassWord(), tg.isVaiTro() ? "NhanVien" : "Admin",
									tg.isTrangThai() ? "Còn Hoạt Động" : "Nghỉ hoạt động" };
							model.addRow(row);
						}
					}
